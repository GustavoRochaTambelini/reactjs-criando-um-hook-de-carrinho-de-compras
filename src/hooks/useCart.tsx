...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      const arrayProdct = [...cart];
      const productExist = arrayProdct.find(product => product.id === productId);
      ...
      if (productExist) {
        ...
        arrayProdct.push(newProduct);
      }
      setCart(arrayProdct);
      localStorage.setItem('@RocketShoes:cart', JSON.stringify(arrayProdct));
    } catch {
